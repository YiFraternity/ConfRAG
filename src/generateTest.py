...
class SeqConfidenceRAG(BasicRAG):
    ...
    def _get_confs_class_(self, question, history_resp, sent, docs, confs_class='value'):
        ...
        def __conf_level_in_confs__(confs):
            conf_levels = ['very certain', 'fairly certain']
            for conf_level in conf_levels:
                if conf_level in confs.lower():
                    return True
            return False
        if self.reflection_threshold < 0:  # no reflect
            if (confs_class == 'value' and confs >= self.hallucination_threshold) or (confs_class == 'level' and __conf_level_in_confs__(confs)):
                return 1
            else:
                return -1
